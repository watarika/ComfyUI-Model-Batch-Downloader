...
| `llm` | `models/llm` | 外部LLM consumer。companion loaderなし |
| `ultralytics_bbox` | `models/ultralytics/bbox` | Impact Subpack |
| `ultralytics_segm` | `models/ultralytics/segm` | Impact Subpack |

embeddingファイルはプロンプト参照で使用し、companion loaderはありません。LLM対応は単一ファイルのダウンロードのみで、リポジトリsnapshotや複数ファイル推論は提供しません。Impact Pack、`comfyui-sam3`、Impact Subpackなどの任意のcustom nodeはこのdownloaderの依存関係ではありません。consumer nodeが必要な場合は別途インストールして使用してください。
...
Publisher IDは`watarika`です。GitHub Secretの`REGISTRY_ACCESS_TOKEN`にComfy RegistryのAPI keyを設定してください。公開は手動で実行できるほか、`main`上で`pyproject.toml`が変更されたときに実行されます。
